  ...
  return (
    <div className="min-h-screen flex flex-col font-sans selection:bg-brand-blue selection:text-white">
      ...
      <nav className={`sticky top-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white shadow-2xl py-2' : 'bg-white py-4'}`}>
        <div className="container mx-auto px-4 sm:px-6">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden lg:flex space-x-8 xl:space-x-12 items-center">
              ...
              <Link to="/contact" className="px-6 xl:px-8 py-3.5 bg-brand-black text-white rounded-2xl font-black uppercase tracking-[0.2em] text-[11px] transition-all duration-300 hover:bg-brand-blue hover:scale-105 active:scale-95 flex items-center gap-2 shadow-lg hover:shadow-brand-blue/30">
                Contact Us
              </Link>
            </div>

            {/* Mobile Menu Button */}
            <button className="lg:hidden p-2 sm:p-3 text-brand-black bg-slate-50 hover:bg-brand-green hover:text-white rounded-xl transition-all" onClick={() => setIsMenuOpen(!isMenuOpen)}>
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="lg:hidden bg-white border-t py-8 sm:py-12 shadow-2xl absolute top-full left-0 w-full animate-in slide-in-from-top-4 z-50 overflow-y-auto max-h-[90vh]">
            <div className="container mx-auto px-6 sm:px-8 flex flex-col space-y-2 sm:space-y-4">
              <Link to="/" className="text-gray-900 font-black uppercase tracking-[0.3em] text-[13px] sm:text-[14px] py-4 sm:py-5 border-b border-gray-100 flex justify-between items-center group" onClick={() => setIsMenuOpen(false)}>
                Home <Globe size={16} className="text-brand-green opacity-0 group-hover:opacity-100 transition-all" />
              </Link>
              <Link to="/about" className="text-gray-900 font-black uppercase tracking-[0.3em] text-[13px] sm:text-[14px] py-4 sm:py-5 border-b border-gray-100 flex justify-between items-center group" onClick={() => setIsMenuOpen(false)}>
                About <Globe size={16} className="text-brand-green opacity-0 group-hover:opacity-100 transition-all" />
              </Link>
              <Link to="/portfolio" className="text-gray-900 font-black uppercase tracking-[0.3em] text-[13px] sm:text-[14px] py-4 sm:py-5 border-b border-gray-100 flex justify-between items-center group" onClick={() => setIsMenuOpen(false)}>
      ...
      <footer className="bg-brand-black text-white/80 pt-16 sm:pt-28 pb-12 border-t border-white/10 relative overflow-hidden">
        <Dna size={800} className="absolute -bottom-64 -left-64 text-white/[0.03] pointer-events-none" />
        <div className="container mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-12 sm:gap-16 mb-16 sm:mb-24">
            <div className="md:col-span-5">
              ...
              <p className="mb-8 sm:mb-12 leading-relaxed text-[15px] sm:text-[16px] max-w-md font-medium">
            ...
            <div className="md:col-span-3">
              <h3 className="text-white text-[10px] sm:text-[11px] font-black mb-8 sm:mb-12 uppercase tracking-[0.4em] flex items-center gap-3">
                <span className="w-6 h-[2px] bg-brand-green"></span> Company Index
              </h3>
              <ul className="space-y-4 sm:space-y-6 text-[14px] sm:text-[15px] font-bold">
            ...
            <div className="md:col-span-4">
              ...
              <div className="space-y-6 sm:space-y-8 text-[14px] sm:text-[15px] font-medium">
          ...
          <div className="border-t border-white/10 pt-8 sm:pt-12 flex flex-col md:flex-row justify-between items-center gap-6 text-[9px] sm:text-[11px] font-black uppercase tracking-[0.3em] text-white/40 text-center md:text-left">
